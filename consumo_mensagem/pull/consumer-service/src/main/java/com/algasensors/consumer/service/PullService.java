package com.algasensors.consumer.service;

import com.algasensors.consumer.dto.PessoaDTO;
import com.algasensors.consumer.dto.ResultadoPullDTO;
...
public class PullService {
    ...
    public ResultadoPullDTO pullTodos() {
        ...
        while (true) {
            // receive() retorna a Message bruta — usamos para inspecionar
            // o campo messageCount do GetResponse, que indica quantas
            // mensagens AINDA RESTAM na fila APÓS esta entrega.
            Message message = rabbitTemplate.receive(queue, receiveTimeout);
            ...
            // Desserializa manualmente usando o converter configurado
            PessoaDTO pessoa = (PessoaDTO) rabbitTemplate
                    .getMessageConverter()
                    .fromMessage(message);

            // messageCount: quantas mensagens restam APÓS a entrega desta.
            // O broker atualiza esse campo a cada basic.get.
            long restantes = message.getMessageProperties().getMessageCount();

            pessoas.add(pessoa);
            log.info("[PULL] Recebida | uuid={} nome={} | restantes na fila={}",
                    pessoa.uuid(), pessoa.nome(), restantes);

            if (restantes == 0) {
                log.info("[PULL] messageCount=0 — fila drenada | total={}", pessoas.size());
                break;
            }
        }
